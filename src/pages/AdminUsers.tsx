...
export function AdminUsers() {
  ...
  const loadUsers = async () => {
    setLoading(true);
    try {
      const res = await getGithubFile({ ...config, path: 'registry/users.json' });
      if (res && res.content) {
      ...
      <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-primary-500 outline-none transition-all"
            value={searchTerm}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                ...
                <tr><td colSpan={5} className="py-20 text-center text-slate-300 italic">Kullanıcı bulunamadı.</td></tr>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400 font-mono">ID: {btoa(u.username).slice(0,8)}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4">
